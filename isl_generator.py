...
class ISLGenerator:
    def __init__(self, gloss_map_path, data_dir):
        ...
        # Question words that should go to the end
    ...
    def apply_isl_grammar(self, sentence):
        """Apply ISL grammar rules: SOV order with colors, negation, questions at end."""
        ...
        filtered_tokens = [word for word, tag in tokens if word not in self.auxiliary_verbs]

        # Normalize pronouns
        normalized_tokens = [self.pronoun_map.get(word, word) for word in filtered_tokens]

        # Apply ISL word order: Subject + Object + Verb + Adjectives + Colors + Negation + Question words
        processed_tokens = self._apply_isl_word_order(normalized_tokens, tags)

        return ' '.join(processed_tokens)

    def _apply_isl_word_order(self, tokens, original_tags):
        """Apply ISL word order: Subject + Object + Verb + Colors/Negation/Questions at end."""
        if len(tokens) <= 1:
            return tokens
        ...
        tag_dict = dict(original_tags)

        for word in tokens:
            tag = tag_dict.get(word, '')
            ...
            # Identify colors
                ...
                colors.append(word)
            # Identify subjects (pronouns and first nouns)
            elif tag == 'PRON' or (tag == 'NOUN' and not subjects and not objects_):
                subjects.append(word)
            # Identify objects (nouns that come after subjects)
            elif tag == 'NOUN':
                objects_.append(word)
            # Identify verbs
            elif tag == 'VERB':
                verbs.append(word)
            # Identify adjectives (non-color)
            elif tag == 'ADJ' and not self._is_color_word(word):
                adjectives.append(word)
            else:
                others.append(word)

        # ISL word order: Subject + Object + Verb + Adjectives + Others + Colors + Negation + Questions
        result = subjects + objects_ + verbs + adjectives + others + colors + negations + questions
        
        print(f"ISL Word Order Breakdown:")
        print(f"  Subjects: {subjects}")
        print(f"  Objects: {objects_}")
        print(f"  Verbs: {verbs}")
    ...
    def _text_to_gloss_sequence(self, text):
        """Convert processed English text to ISL glosses."""
        if not text.strip():
            return []
        ...
        while i < len(words):
            ...
            while j < len(words) and words[j] in node:
                ...
                j += 1
                if '__END__' in node:
                    longest_match = node['__END__']

            if longest_match:
                glosses.append(longest_match)
                i += len(longest_match.split())
            else:
                current_word = words[i]
                
                # Check for common phrases that should be kept together
                if i + 1 < len(words):
                    two_word_phrase = f"{current_word} {words[i+1]}"
                    if two_word_phrase in self.gloss_map:
                        glosses.append(two_word_phrase)
                        i += 2
                        continue
                
                # Single word match
                if current_word in self.gloss_map:
                    glosses.append(current_word)
                else:
                    # Finger-spelling fallback only for unknown words
                    for char in current_word:
                        if char.isalpha() and char in self.gloss_map:
                            glosses.append(char)
                        else:
                            print(f"Warning: No gloss for '{char}', skipping.")
                i += 1
    ...
    def text_to_gloss(self, sentence):
        ...
        # For common greetings, keep them intact
        common_phrases = {
        ...
        # Check if the sentence starts with any common phrase
        for phrase in common_phrases:
            if sentence_lower.startswith(phrase):
                # For common greetings, use minimal processing
                processed_text = sentence_lower
                print(f"Common phrase detected: {phrase}")
                print(f"Preserved text: {processed_text}")
                gloss_sequence = self._text_to_gloss_sequence(processed_text)
                print(f"Gloss sequence: {gloss_sequence}")
                return gloss_sequence
        
        # For other sentences, apply ISL grammar rules
        processed_text = self.apply_isl_grammar(sentence)
    ...
    def _draw_skeleton_on_frame(self, canvas, frame_data):
        ...
        def draw_smiling_face(face_points):
            ...
            if not face_points:
                # Draw default face if no face points
        ...
        # Draw components
        fill_torso(frame_data.get("pose", []))
        ...
        draw_points(pose_data, JOINT_COLOR, size=3)
        draw_points(left_hand_data, JOINT_COLOR, size=3)
        draw_points(right_hand_data, JOINT_COLOR, size=3)
    ...
    def generate_video_from_text(self, text: str) -> str:
        ...
        for token in tokens:
            json_path = self.gloss_map.get(token, None)
...
if __name__ == "__main__":
    ...
    test_sentences = [
        "he has red car",      # Should become: he car red
        "what do you drink",   # Should become: you drink what  
        "she is not sick",     # Should become: she sick not
        "where is my book",    # Should become: my book where
        "good morning teacher" # Should stay: good morning teacher
    ]
